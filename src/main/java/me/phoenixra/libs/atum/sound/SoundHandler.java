package me.phoenixra.libs.atum.sound;
...
public class SoundHandler {
    ...
    public static void init() {
        if (FMLCommonHandler.instance().getSide() != Side.CLIENT) {
            LOGGER.error("[AtumMod] Tried to initialize SoundHandler server-side!");
    ...
    public static void registerSound(String key, String path) {
        ...
        if (FMLCommonHandler.instance().getSide() != Side.CLIENT) {
            LOGGER.error("[AtumMod] Tried to register sound server-side!");
            new Throwable().printStackTrace();
            return;
        }
        if (!sounds.containsKey(key)) {
            ...
            try {
                ...
                LOGGER.error("[AtumMod] Unable to register sound! Format not supported or invalid!");
        ...
        try {
            ...
            if (clip.isOpen()) {
                if (clip.isControlSupported(Type.MASTER_GAIN)) {
                    ...
                    LOGGER.error("[AtumMod] Unable to set sound volume! MASTER_GAIN control not supported! Disabling volume handling!");
                    printErrorLog(new Throwable(), key, percentage, "setting volume");
                }
            } else {
                LOGGER.error("[AtumMod] Unable to set sound volume! Clip not open: " + key);
            }

        } catch (Exception e) {
            volumeHandling = false;
            LOGGER.error("[AtumMod] Unable to set sound volume! Disabling volume handling!");
